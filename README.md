...
> 이 저장소는 프로젝트 중 **하드웨어 구성 및 제어 코드**를 중심으로 정리했습니다.
...
| 개발 환경 | Arduino IDE, OpenPLC, C/C++, RS-232, TCP/IP, Fritzing |
...
PCB 검사 컨베이어, 불량 배출 실린더, 팔레트 이송형 컨베이어, 센서 및 비상정지 버튼, AGV 지게차로 전체 하드웨어를 구성했습니다.
...
정상 PCB는 메인 컨베이어를 따라 계속 이동하고, 불량 PCB는 전기 실린더가 슬라이더 방향으로 밀어내 불량함으로 배출합니다.
...
AI 비전 검사 결과가 불량으로 전달되면 전기 실린더가 PCB를 컨베이어 옆의 불량함으로 밀어냅니다. 배출이 끝난 후 실린더는 원래 위치로 복귀합니다.
...
Pin Mapping에서 전진 출력과 후진 출력을 설정했으며, 실제 장비에서는 불량 PCB를 불량함으로 밀어내는 동작에 사용했습니다.
...
정상 PCB 2개가 박스에 적재되면 팔레트 이송형 컨베이어가 박스를 AGV 지게차의 인수 위치까지 이동시킵니다.
...
AGV 지게차는 PCB가 담긴 박스를 들어 올리고 라인을 따라 창고까지 이동합니다. 운반을 완료하면 초기 위치로 복귀해 다음 작업을 대기합니다.
...
- PCB 검사 컨베이어와 팔레트 이송형 컨베이어 제어
- 적외선 센서를 이용한 PCB 위치 감지
- 불량 판정에 따른 전기 실린더 배출 및 원위치 복귀
- 비상정지 버튼과 부저 경고
- AGV 지게차의 후진, 회전, 전진 및 운반 동작
- PC와 Arduino 사이의 RS-232 통신
- 장치 사이의 시작, 정지, 판정 및 상태 신호 연동
- 전체 장비 통합과 반복 동작 테스트
