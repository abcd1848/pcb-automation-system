...
3. 머신비전 시스템이 PCB 의 부품 누락, 납땜 그을림 검사합니다.
4. 불량 판정이 내려지면 신호를 받고 전기 실린더가 PCB를 옆으로 밀어 불량함으로 배출합니다.
...
| 파일 | 내용 |
|---|---|
| [`AGV.ino`](./AGV.ino) | AGV 동작 제어 코드 |
| [`Conveyor.ino`](./Conveyor.ino) | 컨베이어 동작 제어 코드 |
...
- 지게차 , 컨베이어 , 실린더 전체 설계 제어 코드 로직 구성
